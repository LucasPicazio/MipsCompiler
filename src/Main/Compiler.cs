...
namespace Main
{
    class Compiler
    {
            ...
            for (int i = 0; i < ass.Count; i++)
            {
                ...
                if (Enum.TryParse(opcodeString, true, out OperationEnum operation))
                    switch (operation)
                    {
                        ...
                        case OperationEnum.ADDI://Perguntar: No ep disse que era para usar ADD com constante mas não tem como
        ...
        private BitArray PseudoFormat(OperationEnum operation, List<string> parts)
        {
            ...
            if (operation == OperationEnum.LUI)
                result = LuiOperation(parts);
            else
        ...
        private BitArray LuiOperation(List<string> parts)
        {
